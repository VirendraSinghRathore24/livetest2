              ...
              posts !== null && posts.map((p, index) => (
                <tr className="border-2 border-black">
                  ...
                  <td className="whitespace-wrap text-md text-blue-600 font-medium px-1 py-2 border-r-2 align-baseline text-center break-all text-wrap border-black">
                      <NavLink to={`/testresult?testid=${p.testid}&paper=${p.paper}&&score=${p.result}&&date=${p.date}&&time=${p.timeTaken}&&resultid=${p.resultid}`}>{p.paper}</NavLink>
                  </td>
                  ...
                  <td className="whitespace-wrap text-md font-medium px-1 py-2 border-r-2 align-baseline text-center break-all text-wrap border-black">{p.timeTaken}</td>
                </tr>
                )))
                } 
           </tbody> 
